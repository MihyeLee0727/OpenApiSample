...
class MergedActivity : AppCompatActivity() {

    companion object {
        fun start(context: Context) {
            context.startActivity(Intent(context, MergedActivity::class.java))
        }
    }

    private val adapter = MergedAdapter()
    ...
    private fun initView() {
        with(binding) {
            viewModel = this@MergedActivity.viewModel
            list.adapter = this@MergedActivity.adapter
            ...
            editSearchBox.setOnEditorActionListener { v, actionId, _ ->
                if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                    hideKeyboard(v)
                    this@MergedActivity.viewModel.search()
    ...
    private fun checkReadMore() {
        (binding.list.layoutManager as? LinearLayoutManager)?.let {
            ...
            if (lastItem >= adapter.itemCount - 4) {
                this@MergedActivity.viewModel.readMore()
            }
        }
    }
