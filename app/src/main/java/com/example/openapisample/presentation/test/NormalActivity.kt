...
class NormalActivity : AppCompatActivity() {
    ...
    private fun initView() {
        with(binding) {
            viewModel = this@NormalActivity.viewModel
            list.adapter = this@NormalActivity.adapter
            ...
            editSearchBox.setOnEditorActionListener { v, actionId, _ ->
                if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                    hideKeyboard(v)
                    this@NormalActivity.viewModel.search()
    ...
    private fun checkReadMore() {
        (binding.list.layoutManager as? LinearLayoutManager)?.let {
            ...
            if (lastItem >= adapter.itemCount - 4) {
                this@NormalActivity.viewModel.readMore()
            }
        }
    }
